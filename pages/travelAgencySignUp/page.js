...
export default function TravelAgencySignUpPage(){
    const [travelAgencySignUp,setTravelAgencySignUp] = useState([]);

    useEffect(() => {
        axios.get('/api/travelAgencySignUp').then(response => {
    ...
    return(
        <Layout>
            ...
            <table className="basic mt-2">
                ...
                <tbody>
                    {travelAgencySignUp.map(tasu => (
                        <tr key={tasu._id}>
                            <td>{tasu.ownerFirstName}</td>
                            <td>{tasu.ownerLastName}</td>
                            <td>{tasu.businessEmail}</td>
                            <td>{tasu.ownerMobileNum}</td>
                            <td>{tasu.businessName}</td>
                            <td>{tasu.businessAddress}</td>
                            <td>{tasu.contactNumber}</td>
                            <td>
                                <img src={tasu.birCertificatePhoto} alt="" className="h-20 w-20 object-cover rounded-lg" />
                            </td>
                            <td>
                                <img src={tasu.dtiPermitPhoto} alt="" className="h-20 w-20 object-cover rounded-lg" />
                            </td>
                            <td>
                                <img src={tasu.businessPermitPhoto} alt="" className="h-20 w-20 object-cover rounded-lg" />
                            </td>
                            <td>
                                <img src={tasu.mayorsPermitPhoto} alt="" className="h-20 w-20 object-cover rounded-lg" />
                            </td>
                            <td>
                              {tasu.isVerified ? 'Verified' : 'Not Verified'}
                            </td>
                            <td>
                                <Link href={'/travelAgencySignUp/edit/'+tasu._id}>
                                ...
                                <Link href={'/travelAgencySignUp/delete/'+tasu._id}>
